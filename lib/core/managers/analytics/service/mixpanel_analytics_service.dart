...
class MixPanelAnalyticsService implements AnalyticsService{
  ...
  Future<void> initMixpanel() async {
    _mixpanel = await Mixpanel.init(kDebugMode ? AppConstants.mixpanelTokenKeyDev : AppConstants.mixpanelTokenKeyProd, optOutTrackingDefault: false, trackAutomaticEvents: true);
  }

  @override
  Future buttonViewTracker<T extends BaseModel>({required ButtonAnalyticIdentity event, Map<String, dynamic>? parameters}) async{
    ...
    AppUtils.mockPrintAnalytics(
        eventName: "Mixpanel_${event.name}",
        parameters: param);
  }

  @override
  Future registerSuperProperties<T extends BaseModel>({Map<String, dynamic>? properties}) async{
    if(properties!=null) {
